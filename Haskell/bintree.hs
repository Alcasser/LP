...
height :: Tree a -> Int
height Empty = 0
height (Node n esq dre) = 1 + max (height esq) (height esq) 
...
isomorphic (Node n1 e1 d1) (Node n2 e2 d2) =
     (n1 == n2) && (isomorphic e1 e2 || isomorphic e1 d2 || isomorphic d1 d2 || isomorphic d1 e2)
...
bfs :: Tree a -> [a]
bfs (Node a esq dre) = a : bfs' [esq, dre]
bfs' :: [Tree a] -> [a]
bfs' [] = []
bfs' (Empty : xs) = bfs' xs
